...
sudo xmllint --format --output $(pwd)/licenseFile/UnityRequestFile.alf $(pwd)/licenseFile/Unity_v${UNITY_VERSION}.alf
...
echo -e "\n\n\n" && cat $(pwd)/licenseFile/UnityRequestFile.alf
...
docker kill remoteSelenium  # TODO remove
sudo mv $(pwd)/licenseFile/*.ulf $(pwd)/licenseFile/UnityLicense.ulf
ls $(pwd)/licenseFile
cat $(pwd)/licenseFile/*.ulf
